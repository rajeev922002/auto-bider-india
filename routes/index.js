 ...
 try {
  ...
  console.log("payments : ", payments[0]);
   ...
   if (payments.length === 0) {
    return res.render("index", { title: "Express", payments: [] }); // Pass an empty array or handle accordingly
  }
...
router.get("/admin/totalUsers", isAdmin, async (req, res) => {
  try {
    ...
    console.log("here are total users  ---> ",totalUsers)

    // Calculate total pages for pagination
    const totalPages = Math.ceil(totalUsers / limit);
    console.log("here are total Pages  ---> ",totalPages)
...
router.get("/createAdmin", async (req, res) => {
  ...
  let password = "$2a$12$W5mmBjH9k6QXJb0LT9UDmewRCDkmTAjDUXleye2rfq3LShNeouh76";
